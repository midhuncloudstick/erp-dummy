...
const AdminSupportTicketDetailPage = () => {
  ...
  return (
    <AdminLayout title="Support" subtitle={`Ticket Details - ${ticket.id}`}>
      <div className="space-y-6">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <div className="ml-4">
                    <Select
                      value={ticket.status}
                      onValueChange={(value) => handleStatusChange(value as 'open' | 'in-progress' | 'resolved' | 'closed')}
                    >
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-lg">Initial Response</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="bg-primary/5 border-l-4 border-l-primary rounded-lg p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-xs text-muted-foreground">
                  {formatDate(ticket.messages[0]?.timestamp || ticket.createdAt)}
                </span>
              </div>
              ...
              {/* Initial Response Files */}
              {ticket.messages[0]?.files && ticket.messages[0].files.length > 0 && (
                <div className="space-y-2">
                  <div className="text-xs font-medium text-muted-foreground flex items-center gap-1">
                    <Paperclip className="h-3 w-3" />
                    Attached Files ({ticket.messages[0].files.length})
                  </div>
                  <div className="grid grid-cols-1 gap-2">
                    {ticket.messages[0].files.map((file) => (
                      <div 
                        key={file.id}
            ...
            <div className="space-y-6">
              {ticket.messages.map((message, index) => (
                <div key={message.id} className="space-y-4">
                  ...
                  {/* Add red note between customer messages */}
                  {message.isClient && index < ticket.messages.length - 1 && (
        ...
        {(ticket.status === 'open' || ticket.status === 'in-progress') && (
          <Card>
            <CardHeader>
              <CardTitle>Add Internal Note or Customer Response</CardTitle>
            </CardHeader>
            <CardContent>
