...
import { ArrowLeft, Plus, X, Server, Settings, Loader2, Trash2 } from 'lucide-react';
import { toast } from 'sonner';
import { useAppDispatch, useAppSelector } from '@/store/hooks';
import { getServiceById, updateService, deleteService } from '@/store/slices/serviceSlice';
...
const EditServicePage = () => {
  ...
  // Redux state
  const { currentService, loading, error, deletingService, deleteError } = useAppSelector((state) => state.services);
...
console.log(currentService)
  ...
  useEffect(() => {
    if (serviceId) {
      dispatch(getServiceById(parseInt(serviceId)));
    }
    dispatch(fetchCategories());
  }, [dispatch, serviceId]);

  // Handle service data loading
  useEffect(() => {
    if (currentService) {
      setServiceName(currentService.name);
      setCategoryId(currentService.category_id.toString());
      setPaymentType(currentService.type);
      console.log("paymentType",paymentType)
      
      if (currentService.type === 'onetime' && currentService.one_time_cost) {
        setCost(currentService.one_time_cost);
      } else if (currentService.type === 'recurring') {
        setMonthlyCost(currentService.monthly_cost || '');
        setYearlyCost(currentService.yearly_cost || '');
      }
      
      setDescriptionPoints(currentService.features);
      
      // Load custom fields
      if (currentService.custom_fields) {
        const mappedCustomFields = currentService.custom_fields.map((field, index) => ({
          id: index.toString(),
          fieldName: field.field_name,
          mandatory: field.is_mandatory,
          fieldType: field.field_type,
          options: field.field_select?.map(option => option.options) || []
        }));
  ...
  }, [error, categoryError, deleteError]);
  ...
  const handleDeleteService = async () => {
    ...
    try {
      const result = await dispatch(deleteService(parseInt(serviceId)));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (categoryId === 'new' && newCategory.trim()) {
        const result = await dispatch(createCategory({ category: newCategory.trim() }));
      ...
      const servicePayload: any = {
        name: serviceName,
        type: paymentType === 'onetime' ? 'onetime' : 'recurring',
      ...
      // Update the service
      if (serviceId) {
        const result = await dispatch(updateService({ id: parseInt(serviceId), serviceData: servicePayload })).unwrap();
                ...
                <AlertDialog>
                  <AlertDialogTrigger asChild>
                    <Button
                      ...
                      className="border-red-200 text-red-600 hover:bg-red-50 bg-gray-50"
                    >
                      {deletingService ? (
                        <>
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription>
                        ...
                        "{serviceName}" and remove it from your system.
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
